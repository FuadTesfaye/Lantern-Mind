...
export function PostDiscussion({
  ...
  return (
    <section className="mt-14 border-t border-border/40 pt-12">
      ...
      >
        Talk quietly,{" "}
        <em className="not-italic text-muted-foreground">anonymously.</em>
      </h2>
      <p className="mt-4 max-w-xl text-sm leading-relaxed text-muted-foreground">
        Witness each other. No advice piles, no ranking pain. Messages appear live for
      ...
      <ul className="mt-10 space-y-6">
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <li
              key={comment.id}
              className="animate-fade-rise border-l border-border/50 pl-5"
            >
