...
function DockIcon({
  ...
  return (
    ...
    >
      ...
      {alwaysShowLabels && (
        <span className="mt-0.5 text-[10px] font-medium tracking-tight text-muted-foreground whitespace-nowrap pointer-events-none select-none leading-none">
          {item.label}
        </span>
      )}
    </motion.div>
...
export function Dock({
  ...
  return (
    <motion.div
      ref={dockRef}
      className={cn(
        "relative flex items-end overflow-visible border border-border bg-surface/90 px-3 py-2 shadow-xl backdrop-blur-md",
        className,
      )}
      ...
      {!alwaysShowLabels && (
        <AnimatePresence>
          {hoveredIndex !== null && items[hoveredIndex] && (
            ...
            >
              <span className="rounded-full border border-border bg-card px-2.5 py-1 text-xs font-medium text-foreground shadow-md whitespace-nowrap">
